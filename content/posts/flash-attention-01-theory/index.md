...
3. [FlashAttention-2/3 CUDA 前向实现]({{< relref "flash-attention-03-cuda-fwd" >}}) —— 工业级 kernel 的调度、流水线与 warp specialization
4. [反向梯度推导与 recompute 实现]({{< relref "flash-attention-04-bwd" >}}) —— dQ/dK/dV 的完整推导与三种实现对照
5. [Gluon / TileLang / Liger 多实现对比]({{< relref "flash-attention-05-dsl-zoo" >}}) —— 不同抽象层级下的同一算法
6. [官方仓库源码全景]({{< relref "flash-attention-06-official-code" >}}) —— FA2 / FA3 / FA4 三套实现的目录结构、调用链与构建方式
7. [反向传播深潜]({{< relref "flash-attention-07-bwd-deep" >}}) —— 四个抽象层级的反向实现、MLA 反向与完整的反向数学

本篇不涉及任何具体代码，只回答一个问题：**Flash Attention 到底解决了什么问题，它的数学基础是什么**。读完你应该能独立推导出 online softmax 的递推公式，并理解为什么它是**精确计算**而非近似。

先给一个贯穿全篇的比喻：**softmax 好比给一列数"称重后按比例分配"**。麻烦在于，想给一个数称重，你得先把整列数都放到秤上、找到最大值当"皮重"、算出总和当"分母"。数据少时无所谓，数据多到装不下时就麻烦了——而 Flash Attention 的全部智慧，就是**让这台秤可以一边来货一边称，永远不需要把所有货同时堆在台面上**。
...
以 $N = 8192$、fp16 存储为例，一个 $N \times N$ 矩阵是 **128 MB**——注意这还只是**一个 head、一个 batch**。而这三个操作之间的每一次中间结果读写都发生在 HBM（高带宽显存，A100 上约 2 TB/s）上。GPU 的片上 SRAM（共享内存 + 寄存器）只有几百 KB，比 HBM 快一个数量级以上，但装不下这个矩阵。

打个比方：这就像为了统计"每层楼有几户人家"，却先把整栋楼的门牌号抄成一张 8192×8192 的巨型表格，填一遍、读一遍、再划掉——**表格本身不产生任何价值**，它只是为了让"逐行求和"能做下去。$S$ 和 $P$ 就是这张表。
...
证明只需分子分母同乘 $e^c$。**这个"同乘 $e^c$"就是全部诀窍**：softmax 关心的是"谁占多少比例"，而不是"绝对值多大"，所以给整行数统一加上（或减去）一个常数，比例分毫不变。好比量身高前让所有人站上同一块 10 cm 高的垫子，谁高谁矮的结论不变，但数字都变"友善"了。

取 $c = m_i = \max_k S_{ik}$，则所有指数的参数 $\le 0$，指数值落在 $(0, 1]$，既不上溢、也不下溢。这就是教科书式的三遍扫描（three-pass）softmax：
...
> **小例子（两块走一遍）**。设某行 logits 为 $[1,2,3,4]$，分成 $[1,2]$、$[3,4]$ 两块：
> - 第一块：$m^{(1)}=2$，$\ell^{(1)}=e^{-1}+e^{0}=1.3679$；
> - 第二块到来：$m=4$，修正因子 $\alpha=e^{2-4}=0.1353$，于是 $\ell = \alpha\ell^{(1)} + (e^{3-4}+e^{4-4}) = 0.1851+1.3679=1.5530$；
> - 一次性算法验证：$m=4$，$\ell=\sum_k e^{S_k-4}=e^{-3}+e^{-2}+e^{-1}+e^{0}=1.5530$。**分毫不差**。
>
> 关键动作只有一步：新块来了，把**旧的和乘一个数**（$\alpha$），而不是回头重读第一块。这就是"流式"的全部含义。
...
把 online softmax 装进 GPU，就得到 Flash Attention 的骨架。形象地说，这是一场"**分田到户**"：把 Q 的每个行块分给一个 CTA（一个"工人"），工人拿着一小块 Q，**沿着整条 KV 流水线**逐块处理，边看边更新自己手头的一本小账（$m_i, \ell_i, \tilde{O}$）——全程不必把中间结果搬回仓库（HBM）。
...
论文里的定理值得自己推一遍。把 HBM 想成**大仓库**，把 SRAM 想成**手边的工作台**：朴素实现每做一步都要回仓库取一次料、放一次半成品；分块实现则是"搬一次料够做一个 Q 块，做完再回仓库"。谁跑仓库的次数少，谁就快。设 SRAM 大小为 $M$ 字节，$d \le M/4$（一个 tile 的 K 或 V 装得下）。
