// 48 W4A16 小 M 的 warp specialization（主题 26g）
//
// 46 篇把 M∈[3,16] 的最优交给 `mma.sync.m16n8k16` 的 BM=16 小 tile（`mma_b16_k8`），
// 但它 M=16 时只有 22.6% HBM、Compute 47.7%、L1 40.3%——同一批线程既要做
// cp.async、又要在主循环里把 int4 反量化写进 smem Ba、再 ldmatrix+mma。反量化的
// ALU 与张量核在同一条关键路径上串行。
//
// 本篇把它拆成 producer/consumer 两个 warpgroup：
//   · producer WG：cp.async(W) + int4→bf16 反量化（ALU 重活）→ Ba
//   · consumer WG：只做 ldmatrix + mma
//   · mbarrier full/empty 握手，让反量化与张量核真正重叠
// 与 46 的 `gemm_mma_w4a16_kernel` 逐位等价（同一 smem 布局与 ldmatrix 地址），
// 可与 `mma_b16_k8` 同进程 head-to-head。
//
...
//     scripts/run.sh 48-w4a16-mma-ws/w4a16_mma_ws.cu [which]
...
// ===========================================================================
// (B) 48 篇主角：给 mma_b16 小 tile 叠 warp specialization。
//     producer WG（NPRODW 个）只做 cp.async(W)+反量化→Bs；consumer WG（NCONS 个）
//     只做 ldmatrix+mma。mbarrier full/empty 握手，让反量化的 ALU 与张量核重叠。
//     与 46 的 gemm_mma_w4a16_kernel 逐位等价（同一 smem 布局/ldmatrix 地址）。
// ===========================================================================
template <int BM, int BN, int BKi, int STAGES, int NCONS, int KSPLIT, int NPRODW = 1,
          bool DQ = true>
__global__ void __launch_bounds__((NPRODW + NCONS) * 128) mma_ws_kernel(
    const bf16* __restrict__ A, const uint8_t* __restrict__ Wp, const float* __restrict__ Ws,
    float* __restrict__ C, int M, int N, int K) {
  constexpr int NPROD = NPRODW * 128;
  constexpr int T = (NPRODW + NCONS) * 128;
  constexpr int WN = NCONS * 4;         // 每个 consumer WG 4 个 warp
  constexpr int WARP_N = BN / WN;
  static_assert(WARP_N % 16 == 0, "WARP_N 需按 16 切");
  constexpr int MTN = WARP_N / 8;
  constexpr int ASP = BKi + 8;
  constexpr int BKP = BKi + 8;
  constexpr int NGRP = 5120 / GROUP;
  constexpr int NW4 = BKi / 8;          // 每 n 行的 uint32 数
  constexpr int M4 = BKi / 8;
  constexpr int ABYTES = BM * ASP * 2;
  constexpr int WB_BYTES = BN * (BKi / 2);
  constexpr int SW_BYTES = BN * BKP * 2;

  extern __shared__ __align__(1024) char smem[];
  uint64_t* full = reinterpret_cast<uint64_t*>(smem);
  uint64_t* empty = full + STAGES;
  char* As = smem + ((2 * STAGES * 8 + 1023) / 1024) * 1024;
  char* Wraw = As + (size_t)STAGES * ABYTES;
  char* Bs = Wraw + (size_t)STAGES * WB_BYTES;
  ...
  if (tid < NPROD) {
    auto load_stage = [&](int st, int kb) {
      const int k0 = kb * BKi;
      bf16* a = reinterpret_cast<bf16*>(As + (size_t)st * ABYTES);
      char* wp = Wraw + (size_t)st * WB_BYTES;
      for (int i = tid; i < BM * (BKi / 8); i += NPROD) {
        const int r = i / (BKi / 8), c8 = (i % (BKi / 8)) * 8;
        const int gr = block_row + r;
        if (gr < M)
          __pipeline_memcpy_async(&a[r * ASP + c8], &A[(size_t)gr * K + k0 + c8], 16);
      ...
      for (int i = tid; i < BN * ((BKi / 2) / 16); i += NPROD) {
        const int r = i / ((BKi / 2) / 16), c = (i % ((BKi / 2) / 16)) * 16;
        __pipeline_memcpy_async(&wp[r * (BKi / 2) + c],
                                &Wp[(size_t)(block_col + r) * (K / 2) + k0 / 2 + c], 16);
      }
    ...
    auto dequant_stage = [&](int st, int kb) {
      ...
      char* wp = Wraw + (size_t)st * WB_BYTES;
      bf16* b = reinterpret_cast<bf16*>(Bs + (size_t)st * SW_BYTES);
      const int g = (kb * BKi) / GROUP;  // BKi=64 整除 GROUP=128 → 单 group
      for (int i = tid; i < BN * M4; i += NPROD) {
        const int n = i / M4, w = i % M4;
        const uint32_t word = *reinterpret_cast<const uint32_t*>(&wp[n * (BKi / 2) + w * 4]);
        const float s = __ldg(&Ws[(size_t)(block_col + n) * NGRP + g]);
        bf16 out[8];
#pragma unroll
...
#pragma unroll
    ...
    const int cw = warp - NPRODW * 4;   // 0 .. NCONS*4-1
    const int warp_col = cw % WN;
    float acc[1][MTN][4];
#pragma unroll
    for (int j = 0; j < MTN; ++j)
#pragma unroll
    ...
    for (int t = 0; t < nblk_z; ++t) {
      ...
      const bf16* a = reinterpret_cast<const bf16*>(As + (size_t)s * ABYTES);
      const bf16* b = reinterpret_cast<const bf16*>(Bs + (size_t)s * SW_BYTES);
#pragma unroll
      for (int kk = 0; kk < BKi / 16; ++kk) {
        uint32_t af[4];
        {
          const int row = (lane & 15);
          const int col = (lane >> 4) * 8;
          ldmatrix_x4(smem_u32(&a[row * ASP + kk * 16 + col]), af);
        }
        uint32_t bf[MTN][2];
#pragma unroll
        for (int g = 0; g < MTN / 2; ++g) {
...
#pragma unroll
        for (int j = 0; j < MTN; ++j) mma_m16n8k16(acc[0][j], af, bf[j]);
      }
      mbar_arrive(empty + s);
    }
    const int group = lane >> 2, tig = lane & 3;
...
#pragma unroll
      for (int q = 0; q < 4; ++q) {
        ...
        const int c = c0 + (q & 1);
        if (r < M) {
          if (KSPLIT == 1) C[(size_t)r * N + c] = acc[0][j][q];
          else atomicAdd(&C[(size_t)r * N + c], acc[0][j][q]);
        }
      }
    }
  }
...
template <int BM, int BN, int STAGES, int NCONS, int KSPLIT = 1>
static void add_mma_ws(std::vector<Spec>& specs, const char* name, const Buf& buf, int N, int K) {
  auto fn = mma_ws_kernel<BM, BN, BK, STAGES, NCONS, KSPLIT, 1, true>;
  const int NT = (1 + NCONS) * 128;
  const size_t shm =
      1024 + (size_t)STAGES * (BM * (BK + 8) * 2 + BN * (BK / 2) + BN * (BK + 8) * 2);
  CUDA_CHECK(cudaFuncSetAttribute(fn, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)shm));
  specs.push_back({name, BM, [=](int M) {
                     if (KSPLIT > 1) cudaMemsetAsync(buf.C, 0, (size_t)M * N * 4);
                     fn<<<dim3(N / BN, div_up(M, BM), KSPLIT), NT, shm>>>(
...
int main(int argc, char** argv) {
  ...
  const int CHK_M = (argc > 3) ? std::atoi(argv[3]) : MAXM;  // 可覆盖（便于 ncu 定点剖析）
  ...
  add_mma<16, 128, 2, 1, 4, 12>(specs, "mma_b16_k12", buf, N, K);
  add_mma<16, 128, 2, 1, 4, 16>(specs, "mma_b16_k16", buf, N, K);
  add_mma<16, 128, 2, 1, 4, 20>(specs, "mma_b16_k20", buf, N, K);
  add_mma<16, 128, 2, 1, 4, 24>(specs, "mma_b16_k24", buf, N, K);
  add_mma<16, 128, 2, 1, 4, 32>(specs, "mma_b16_k32", buf, N, K);
  add_mma<16, 128, 3, 1, 4, 16>(specs, "mma_b16_s3k16", buf, N, K);
  add_mma<32, 128, 2, 2, 4, 16>(specs, "mma_b32_k16", buf, N, K);
  add_mma<16, 128, 3, 1, 4, 8>(specs, "mma_b16_s3k8", buf, N, K);
  add_mma<16, 128, 2, 1, 8, 8>(specs, "mma_b16_w8k8", buf, N, K);
  add_mma<16, 128, 3, 1, 8, 8>(specs, "mma_b16_w8s3k8", buf, N, K);
  ...
  // 48 篇：warp-specialized 版（1 producer WG 反量化 + NCONS consumer WG 只做 mma）
  add_mma_ws<16, 128, 2, 1, 1>(specs, "ws_b16_s2c1k1", buf, N, K);
  add_mma_ws<16, 128, 2, 1, 4>(specs, "ws_b16_s2c1k4", buf, N, K);
  add_mma_ws<16, 128, 2, 1, 8>(specs, "ws_b16_s2c1k8", buf, N, K);
  add_mma_ws<16, 128, 3, 1, 8>(specs, "ws_b16_s3c1k8", buf, N, K);
  add_mma_ws<16, 128, 3, 1, 5>(specs, "ws_b16_s3c1k5", buf, N, K);
  add_mma_ws<16, 128, 2, 2, 8>(specs, "ws_b16_s2c2k8", buf, N, K);
  add_mma_ws<16, 128, 3, 2, 8>(specs, "ws_b16_s3c2k8", buf, N, K);
