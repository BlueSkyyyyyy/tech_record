...
- [ ] **文章 29 — 主题 20b-2：gate GEMM + TMA cluster multicast（当前主线）**：把 26 篇的 **A 广播**叠到 gate 上——`cluster.x=3`（grid.x=3 恰是同一 m-tile 的 3 个 n-tile），leader 发 `cp.async.bulk.tensor.2d...multicast::cluster` 把 A 广播给整组，A 的 L2 流量降到 1/3，目标把 32k 的 600 → 700+（逼近 cuBLAS 752）。注意 26 篇的教训：`grid.x % CN == 0`、共享 operand 用 `mapa` arrive、私有/共享 empty barrier 分离。
- [ ] **文章 30 — 主题 20c MoE（三）：fused MoE（gather 版 grouped GEMM + 激活 + unpermute）**：把 27 篇物化的 `permuted_x`（1.41 GB 写 + 读）**省掉**——让 grouped GEMM 直接按 `pos` gather 输入、算完直接加权写回 token（unpermute 融合进 epilogue），目标是砍掉端到端 69% 的搬运的大头。
- [ ] **文章 30 — 主题 21c MoE grouped + per-block FP8 缩放**：把 DeepSeek 真实的 `128×128` weight_block scale 叠进 grouped kernel（24 篇的寄存器墙 + 25 篇的 L2 放大），或改成 PSUM/DeepGEMM 的 scale 布局。
...
- [ ] **其他模型场景**：RoPE/RMSNorm/compressor…（见上）。模型类文章优先；20 的 SW128 与主题 31/38/39 共用基础设施，是性能主线。
  - **编号说明**：「系列大纲」里的数字是**主题编号**（稳定 ID），文章 `NN` 是**发布顺序**。主题 24（MuonClip）= 文章 17、主题 17 上半 = 文章 18、下半 = 文章 19、主题 22（FP8 GEMM 一）= 文章 22、主题 23/32/33（FP8 GEMM 二 · TMA）= 文章 23、主题 23b（FP8 GEMM 三 · per-block 寄存器墙）= 文章 24、主题 21（MoE grouped GEMM）= 文章 25、主题 21b（cluster multicast）= 文章 26、主题 20（MoE router）= 文章 27。**下一篇是文章 28**（主题 20b：wgmma/TMA 版 gate GEMM）；发布时把「文章号 ↔ 主题号」记进「当前进度」。
