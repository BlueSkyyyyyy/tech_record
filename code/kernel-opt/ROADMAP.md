...
- [ ] **文章 40 — 主题 25c：MLA decode 的 wgmma + FP8 张量核（当前主线）**：39 篇诊断出两堵墙——① `ldmatrix` 的 `short_scoreboard`（Q 片段每 tile 重读）② 换 FP8 存储却没换算力。两条都靠 `wgmma` 解：用 smem 描述符喂操作数（砍掉全部 `ldmatrix`）+ **FP8 `wgmma.m64nNk32`**（QK 的 K 天然 K-major，与 20/22 篇 SW128 atom 同构）。shape 同 39（`H=128, DC=512, DR=64, DV=512`）。目标：bf16 ≥ 300 TFLOPS、FP8 KV 真正兑现（≥ 250 GB/s/ms 级）。参考 `~/github/FlashMLA`（sm90 decode）与 `22/23` 篇的 TMA+wgmma 基础设施。代码 `code/kernel-opt/40-mla-decode-wgmma/`。
- [ ] **文章 41 — 主题 26：W4A16 dequant-GEMM（AWQ/GPTQ 风格）**：真实 shape 取自 `/ssd/models/DeepSeek-V4.1-Flash/config.json`（`weight_block=[32,32]`, `expert_dtype=fp4`）。权重按 4-bit group 打包（`int4` + group scale/zero），`dequant→GEMM` 融合：折进 TMA/`cp.async` 之后的 smem→寄存器路径，避免物化 fp16 权重；对标 marlin / vLLM 的 awq kernel，给相对 cuBLAS bf16 的差距。
