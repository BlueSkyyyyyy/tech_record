...
- [ ] **25c MLA decode（wgmma + FP8 张量核）**：解 39 篇的 `ldmatrix`/寄存器墙；参考 FlashMLA sm90 decode
...
- [ ] **文章 41 — 主题 25c：MLA decode 的 wgmma + FP8 张量核（当前主线）**：39 篇诊断出两堵墙——① `ldmatrix` 的 `short_scoreboard`（Q 片段每 tile 重读）② 换 FP8 存储却没换算力。两条都靠 `wgmma` 解：用 smem 描述符喂操作数（砍掉全部 `ldmatrix`）+ **FP8 `wgmma.m64nNk32`**（QK 的 K 天然 K-major，与 20/22 篇 SW128 atom 同构）。shape 同 39（`H=128, DC=512, DR=64, DV=512`）。目标：bf16 ≥ 300 TFLOPS、FP8 KV 真正兑现（≥ 250 GB/s/ms 级）。参考 `~/github/FlashMLA`（sm90 decode，`BLOCK_SIZE_M=64`、2 warpgroup 轮流吃 KV block + `remoteP` 共享 P）。注意 40 篇启发的另一条 W4A16 收尾：**warp-specialized dequant**（把反量化税从 46% 压下去）。代码 `code/kernel-opt/41-mla-decode-wgmma/` / `code/kernel-opt/40b-w4a16-ws/`。
- [ ] **文章 42（候补）— 主题 26b：W4A16 的 warp specialization**：40 篇反量化税 46%、骨架也仅 22% HBM。把 dequant 放进独立 warpgroup/生产者（named barrier 握手），让 `wgmma` 与反量化真正重叠；并试 `m16` 小 tile 打真正的 `M=1` decode。目标把有效权重带宽从 400 GB/s 推向 1.5–2 TB/s、逼近 Marlin。
