...
- [ ] **文章 38 — 主题 25：Paged KV-cache / flash-decoding 推理注意力（当前主线）**：Qwen3 系列 GQA（q=40/kv=8、32/4、64/4，head_dim=128，te-perf 已列）、Kimi-K2.6/DeepSeek-V4 的 MLA 都可退化成 MQA。实现变长 seqlen + **paged block table** 的 decode 注意力（flash-decoding：把 KV 沿序列维 split 成多块，每块 online-softmax 部分归约，再跨块合并）+ 可选 prefill。对标 `~/github/flashinfer` / `vllm` 的 paged attention；给 decode 的 HBM 带宽占比与相对 SOTA 差距。代码 `code/kernel-opt/38-paged-kv/`。**注意**：KV cache 布局（`[num_blocks, block_size, kv_head, head_dim]`）与 gather 的合并访存是重点；参考 flashinfer 的 `BatchDecodeWithPagedKVCache` 与 `SingleDecodeWithPagedKVCache`。
