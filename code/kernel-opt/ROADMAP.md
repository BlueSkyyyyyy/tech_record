...
- [ ] **文章 37（候补）— 主题 21e：把 K1 推向 DeepGEMM 的 1-warpgroup / 248-reg per-block 重叠**：34 篇 K1 只有 78% HBM（k2 是 88.9%），卡在 1 CTA/SM + 每 128-k `wgmma.wait0` 折算排空。DeepGEMM 用 BLOCK_M=128、1 个 math warpgroup、`setmaxnreg 248` + `warpgroup_fence_operand`/`warpgroup_wait<0>` 把折算与下一块 wgmma 重叠。目标 K1 ≥ 88% HBM。
