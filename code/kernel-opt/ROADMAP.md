...
- [ ] **文章 33 — 主题 39 续：MLA 极限冲刺（三）· producer warp 专门化 V 装载 + 单缓冲 vs（当前主线）**：32 篇已证 MN-major 免转置不可行、跨迭代软流水被 ptxas 串行化。改走 **warp specialization**：T=384（消费者 2 warpgroup 只做 QK/softmax/P/PV，生产者 1 warp 用 `barrier` 在「PV 刚读完 vs」到「下轮 PV 前」的长窗口里把 V(i+1) gather 进同一块 vs（**单缓冲即可**，无需双缓冲），用 named barrier/`bar.sync` 做 producer↔consumer 握手；关键是把现在「所有线程同步 V 装载」从关键路径拿掉，让消费者立刻进入 QK(i+1)。注意 KT=64 下 vs 单缓冲 65.5KB、smem 已 216KB，须保持 1 CTA/SM。目标 Sk=8k 从 198.8 推向 230+。代码 `code/kernel-opt/33-mla-v-ws/`。**（本轮优先做了更稳的 MoE+FP8=文章 34；本篇因 MLA 的 smem/寄存器硬阻塞暂缓，见「阻塞」，等 1-warpgroup/248-reg 或 Blackwell 再攻。）**
- [ ] **文章 37（候补）— 主题 21e：把 FP8 per-block 推向 DeepGEMM 的 1-warpgroup / 248-reg 重叠**：24/31/34/36 四篇都卡在同一堵墙——per-block 的 `fin`(+64 regs) 让 288 线程越过 2-CTA 门槛（113），occ 掉到 1 CTA/SM，tensor pipe 从 62%→46-51%、主导 stall 从 `long_scoreboard` 变 `barrier`（每 128-k 块 `wgmma.wait0` 排空）。DeepGEMM 用 BLOCK_M=128、**1 个 math warpgroup（128 线程）**、`setmaxnreg 248` 把 `final_accum(128)+accum(64)=192` 塞下且不 spill，配 `warpgroup_fence_operand`/`warpgroup_wait<0>` 避开 ptxas 串行化。目标：把 36 篇的 FP8 per-block 投影从 **959** 推向 1300+（≥65% FP8 峰值），并回填 24/31/34。代码 `code/kernel-opt/37-fp8-pb-1wg/`。
...
- [ ] **其他模型场景**：RoPE/RMSNorm/compressor…（见上）。模型类文章优先；20 的 SW128 与主题 31/38/39 共用基础设施，是性能主线。
  - **编号说明**：「系列大纲」里的数字是**主题编号**（稳定 ID），文章 `NN` 是**发布顺序**。主题 24（MuonClip）= 文章 17、主题 17 上半 = 文章 18、下半 = 文章 19、主题 22（FP8 GEMM 一）= 文章 22、主题 23/32/33（FP8 GEMM 二 · TMA）= 文章 23、主题 23b（FP8 GEMM 三 · per-block 寄存器墙）= 文章 24、主题 21（MoE grouped GEMM）= 文章 25、主题 21b（cluster multicast）= 文章 26、主题 20（MoE router）= 文章 27、主题 20b（gate wgmma/TMA）= 文章 28、主题 20b-2（gate cluster multicast）= 文章 29、主题 20c（fused MoE）= 文章 30、主题 21c（MoE grouped + per-block FP8）= 文章 31、主题 19（RMSNorm/QK-Norm）= 文章 32、主题 21d（fused MoE + per-block FP8）= 文章 34、主题 17 剩余（DSA compressor）= 文章 35。**下一篇候选：文章 36**（主题 17b：compressor 投影的 L2 墙 · B 侧 multicast/L2 常驻）**或文章 33**（主题 39 续：MLA 极限冲刺三 · producer warp 专门化 V 装载，见「阻塞」）**或文章 37**（主题 21e：K1 的 1-warpgroup/248-reg per-block 重叠 / paged KV-cache）。发布时把「文章号 ↔ 主题号」记进「当前进度」。
