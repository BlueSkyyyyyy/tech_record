...
- [ ] **文章 21 — 主题 32/33/39（下一步，优先）MLA 极限冲刺（二）：TMA + 多级流水 + warp specialization**：解决 20 暴露的结构瓶颈（1 CTA/SM + 单缓冲 + 无 producer/consumer）。①用 `cp.async.bulk.tensor.2d`（TMA）+ `mbarrier` 把 Q/K/V 搬运与 tensor 计算解耦（多级 stage）；②warp specialization（producer warp 搬数据 / consumer warp 跑 wgmma），让 tensor pipe 从 18.5% 提到 ≥40%；③可选：把 V 的 K-major 转置改成 TMA im2col/或在 P 侧重排。目标稠密 MLA **250+ TFLOPS**（与 FlashMLA 差距 ≤2.6×），SW128 描述符基础设施直接复用。若 TMA 路径仍受 smem 容量限制，则先做「KT=32 + 双缓冲 K/V」的中间版验证流水收益。
- [ ] **文章 22 — 主题 20/21 MoE：router + top-k + permutation / grouped GEMM**：从 `/ssd/models/DeepSeek-V4-Pro/config.json`（384 routed + 1 shared, top-6, moe_inter=3072）构造 shape，对照 `~/github/DeepGEMM` 的 contiguous/masked 分组，实测路由 + 搬运开销与 grouped GEMM 吞吐。
