...
- [ ] **文章 30 — 主题 20c MoE（三）：fused MoE（gather 版 grouped GEMM + 激活 + unpermute）（当前主线）**：把 27 篇物化的 `permuted_x`（1.41 GB 写 + 读）**省掉**——让 grouped GEMM 直接按 `pos` gather 输入、算完直接加权写回 token（unpermute 融合进 epilogue），目标是砍掉端到端 69% 的搬运的大头。
...
- [ ] **其他模型场景**：RoPE/RMSNorm/compressor…（见上）。模型类文章优先；20 的 SW128 与主题 31/38/39 共用基础设施，是性能主线。
  - **编号说明**：「系列大纲」里的数字是**主题编号**（稳定 ID），文章 `NN` 是**发布顺序**。主题 24（MuonClip）= 文章 17、主题 17 上半 = 文章 18、下半 = 文章 19、主题 22（FP8 GEMM 一）= 文章 22、主题 23/32/33（FP8 GEMM 二 · TMA）= 文章 23、主题 23b（FP8 GEMM 三 · per-block 寄存器墙）= 文章 24、主题 21（MoE grouped GEMM）= 文章 25、主题 21b（cluster multicast）= 文章 26、主题 20（MoE router）= 文章 27、主题 20b（gate wgmma/TMA）= 文章 28、主题 20b-2（gate cluster multicast）= 文章 29。**下一篇是文章 30**（主题 20c：fused MoE）；发布时把「文章号 ↔ 主题号」记进「当前进度」。
