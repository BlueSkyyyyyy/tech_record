...
- [ ] **文章 28 — 主题 20b MoE（一·续）：`wgmma`/TMA 版 gate GEMM**（当前主线）：把 27 篇的 gate GEMM 从 `mma.sync`（204 TFLOPS，27.7% cuBLAS）换成 **`wgmma` SS + SW128**（复用 20/22 篇基础设施）→ 目标 ≥ 450 TFLOPS；再加 **TMA + warp specialization**（复用 23 篇）→ 冲 cuBLAS 的 80%。这直接决定 MoE 前门端到端（27 篇 gate 占 27%）。
- [ ] **文章 29 — 主题 20c MoE（三）：fused MoE（gather 版 grouped GEMM + 激活 + unpermute）**：把 27 篇物化的 `permuted_x`（1.41 GB 写 + 读）**省掉**——让 grouped GEMM 直接按 `pos` gather 输入、算完直接加权写回 token（unpermute 融合进 epilogue），目标是砍掉端到端 69% 的搬运的大头。
