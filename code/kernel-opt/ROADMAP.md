...
- [ ] **文章 31 — 主题 21c MoE grouped + per-block FP8 缩放（当前主线）**：把 DeepSeek 真实的 `128×128` weight_block scale 叠进 [25 篇](25-moe-grouped-gemm/)的 grouped kernel（24 篇的寄存器墙 + 25 篇的 L2 放大），或改成 PSUM/DeepGEMM 的 scale 布局；可在 30 篇的 fused 骨架上直接做（gather + per-block FP8 down GEMM + unpermute）。
