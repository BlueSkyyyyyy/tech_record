...
- [ ] **文章 22 — 主题 32/33/39（下半，下一步，优先）MLA 极限冲刺（三）：TMA + 双缓冲 + warp specialization**：解决 21 遗留的 V 预取与 1 CTA/SM。①`cp.async.bulk.tensor.2d`（TMA）+ `mbarrier` 做 K/V 多级 stage（smem 放不下双份 K+V → 需先把 Q（72KB）挪出 smem 或用 TMA 按需装载 / 降 KT 到 32 配 SW64）；②warp specialization（producer 搬数据 / consumer 跑 wgmma），把 tensor pipe 从 30% 提到 ≥40%；③**消 QK 的 2× 重复**（两 WG 现各算一份完整 S）或改 `wgmma` RS 让 P 从寄存器直接喂 PV（省 P 的 smem 往返 + 一次 `__syncthreads`）。目标稠密 MLA **250+ TFLOPS**（与 FlashMLA 差距 ≤2.6×）。SW128 基础设施复用。
- [ ] **文章 23 — 主题 20/21 MoE：router + top-k + permutation / grouped GEMM**：从 `/ssd/models/DeepSeek-V4-Pro/config.json`（384 routed + 1 shared, top-6, moe_inter=3072）构造 shape，对照 `~/github/DeepGEMM` 的 contiguous/masked 分组，实测路由 + 搬运开销与 grouped GEMM 吞吐。
