...
- [ ] **文章 36（候补）— 主题 17 剩余：DSA compressor（ratio ∈ {4,128,0}）**：V4-Pro/Kimi 的层间 KV 压缩算子，接 indexer/top-k（见 backlog）。
...
- [ ] **其他模型场景**：RoPE/RMSNorm/compressor…（见上）。模型类文章优先；20 的 SW128 与主题 31/38/39 共用基础设施，是性能主线。
  - **编号说明**：「系列大纲」里的数字是**主题编号**（稳定 ID），文章 `NN` 是**发布顺序**。主题 24（MuonClip）= 文章 17、主题 17 上半 = 文章 18、下半 = 文章 19、主题 22（FP8 GEMM 一）= 文章 22、主题 23/32/33（FP8 GEMM 二 · TMA）= 文章 23、主题 23b（FP8 GEMM 三 · per-block 寄存器墙）= 文章 24、主题 21（MoE grouped GEMM）= 文章 25、主题 21b（cluster multicast）= 文章 26、主题 20（MoE router）= 文章 27、主题 20b（gate wgmma/TMA）= 文章 28、主题 20b-2（gate cluster multicast）= 文章 29、主题 20c（fused MoE）= 文章 30、主题 21c（MoE grouped + per-block FP8）= 文章 31、主题 19（RMSNorm/QK-Norm）= 文章 32、主题 21d（fused MoE + per-block FP8）= 文章 34。**下一篇候选：文章 33**（主题 39 续：MLA 极限冲刺三 · producer warp 专门化 V 装载，见「阻塞」）**或文章 35**（主题 21e：K1 的 1-warpgroup/248-reg per-block 重叠，或 DSA compressor / paged KV-cache）。发布时把「文章号 ↔ 主题号」记进「当前进度」。
