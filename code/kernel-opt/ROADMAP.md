...
- [ ] **文章 26 — 主题 21b MoE（二·续）：TMA cluster multicast 压 L2**（下一步，优先）：25 篇 ncu 显示 L2 82%、DRAM 52%，B 的 L2 流量放大 ~4.5×。给 grouped kernel 加 **cluster（同 expert 相邻 m-tile 组队）+ `cp.async.bulk.tensor.2d...multicast::cluster`**，B tile 从 L2 只取一次广播给 cluster 内多个 SM；变长 group 用 DeepGEMM 式 `is_peer_cta_alive` 动态关 multicast。目标把 prefill 从 51% 推向 60%+（tensor pipe 成为新瓶颈）。
- [ ] **文章 27 — 主题 20 MoE（一）：router + top-k + permutation**（可并行）：384 experts top-6 的 router GEMM、top-k（复用 18 篇 radix-select）、token permute/unpermute，实测路由与搬运开销。
- [ ] **文章 28 — 主题 21c MoE grouped + per-block FP8 缩放**：把 DeepSeek 真实的 `128×128` weight_block scale 叠进 grouped kernel（24 篇的寄存器墙 + 25 篇的 L2 放大），或改成 PSUM/DeepGEMM 的 scale 布局。
...
- [ ] **其他模型场景**：RoPE/RMSNorm/compressor…（见上）。模型类文章优先；20 的 SW128 与主题 31/38/39 共用基础设施，是性能主线。
  - **编号说明**：「系列大纲」里的数字是**主题编号**（稳定 ID），文章 `NN` 是**发布顺序**。主题 24（MuonClip）= 文章 17、主题 17 上半 = 文章 18、下半 = 文章 19、主题 22（FP8 GEMM 一）= 文章 22、主题 23/32/33（FP8 GEMM 二 · TMA）= 文章 23、主题 23b（FP8 GEMM 三 · per-block 寄存器墙）= 文章 24、主题 21（MoE grouped GEMM）= 文章 25。**下一篇是文章 26**；发布时把「文章号 ↔ 主题号」记进「当前进度」。
