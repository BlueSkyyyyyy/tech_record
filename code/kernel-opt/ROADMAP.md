...
- [x] **文章 34 — 主题 21d：fused MoE + per-block FP8（grouped per-block up/gate + swiglu/quant + down）**：**已完成并发布**（见「当前进度」）。把 31 的 per-block grouped 接进 30 的 MoE FFN 端到端：权重 50.7→**25.4 GB**，端到端相对 bf16 **1.92×（M8k bal）/ 2.21×（M16k）/ 2.06×（M8k rand）**；K2 down **88.9% HBM**。两个负结果：①**unpermute 融合在 FP8 下失效**（K2f 3.83 vs 3.35+0.54，L2 94.4%/DRAM 73.2%）——GEMM 变快后散写不再被藏住；②**per-block + 双累加器 SwiGLU = 256>255 寄存器**，`BN=256` 崩到 21.7 ms。附带 swiglu+动态量化从 39%→**92% HBM**（一行一 block/float4/warp amax）。代码 `34-fused-moe-fp8/`。
