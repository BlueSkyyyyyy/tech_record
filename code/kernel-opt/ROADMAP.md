...
- [ ] **文章 35（候补）— 主题 21e：把 K1 推向 DeepGEMM 的 1-warpgroup / 248-reg per-block 重叠**：34 篇 K1 只有 78% HBM（k2 是 88.9%），卡在 1 CTA/SM + 每 128-k `wgmma.wait0` 折算排空。DeepGEMM 用 BLOCK_M=128、1 个 math warpgroup、`setmaxnreg 248` + `warpgroup_fence_operand`/`warpgroup_wait<0>` 把折算与下一块 wgmma 重叠。目标 K1 ≥ 88% HBM。
- [x] **文章 36（候补）— 主题 17 剩余：DSA compressor（ratio ∈ {4,128,0}）**：**已完成并发布为文章 35**（见「当前进度」）。合并投影 650–655 TFLOPS（65–67%，cuBLAS 的 ~80%）+ 融合池化 83–84% HBM，端到端 2.0–2.4×；投影被 L2 卡住（见「下一步」）。
- [ ] **文章 36（新）— 主题 17b：compressor 投影的 L2 墙 —— B 侧 cluster multicast / 分块 L2 常驻**：35 篇 ncu 判决投影 `L2` 85–88% 受限（`Compute` 仅 57–59%），根因权重 `Wm[2C,D]`（29–58 MB）被 `M/BM=128` 个 m-tile 重读、L2 读 ≈7.5 GB。两条路：① 沿 M 的 `cluster.y` 广播 B（同 n-tile 的相邻 m-tile 组队），注意 26/29 篇的判据——先看 `lts__t_sectors` 而非 `lts__throughput`，且 `grid.y % CN` 要整除；② 用 `cudaAccessPolicyWindow`（persisting）把 `Wm` 钉进 L2（ratio128 时 29MB < 50MB L2），看能否降 DRAM/L2 抖动。目标投影 ≥ 720 TFLOPS（cuBLAS 的 90%+）。代码 `36-dsa-compressor-mcast/`。
