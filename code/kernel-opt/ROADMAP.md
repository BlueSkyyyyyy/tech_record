...
- [ ] **文章 39 — 主题 25b：MLA 吸收式 decode + FP8 KV-cache（当前主线）**：把 38 的 paged decode 框架接到 DeepSeek-V4 / Kimi 的 MLA 吸收形式（`Hkv=1` 的 MQA；QK 维度 = `kv_lora_rank + qk_rope = 512+64=576`（V4-Pro）/`512+64`（Kimi），V 维度 = `kv_lora_rank = 512`）。要点：①Q 不是单头而是 `Hq` 个 head 共享同一份 `c_kv`（MQA，等同 38 的 G=Hq，但 M=16 只够 16 个 head，V4-Pro `Hq=128` 要 M 方向再分块）；②QK 与 V 维度不同，`mma` 的 N/K 要拆两个循环；③**FP8 KV-cache**：KV 用 `e4m3 + per-tensor/per-channel scale`（decode 是带宽受限，字节减半直接 ≈ 2×），对标 vLLM/FlashMLA 的 fp8 kv。shape 取自 `/ssd/models/DeepSeek-V4-Pro/config.json`（H=128, DC=512, DR=64, DV=512）。代码 `code/kernel-opt/39-mla-decode/`。
- [ ] **文章 40 — 主题 26：W4A16 dequant-GEMM（AWQ/GPTQ 风格）**：真实 shape 取自 `/ssd/models/DeepSeek-V4.1-Flash/config.json`（`weight_block=[32,32]`, `expert_dtype=fp4`）与 `/ssd/models/*/config.json`。权重按 4-bit group 打包（`int4` + group scale/zero），decode/prefill 的 `dequant→GEMM` 融合：把反量化折进 `cp.async`/TMA 之后的 smem→寄存器路径，避免物化 fp16 权重；对标 `~/github` 下的 marlin / vLLM 的 awq kernel。给相对 cuBLAS bf16 的差距。
