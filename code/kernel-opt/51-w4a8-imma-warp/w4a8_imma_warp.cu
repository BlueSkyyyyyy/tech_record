// 51 W4A8 IMMA 小 M GEMM：warp-private pipeline——把 `barrier` 墙拆到零（主题 26j）
//
// 50 篇把墙从 `long_scoreboard`（1.71）换成了 `barrier`（1.61）：128 线程/4 warp 共享
// 同一批 A/W stage，每 stage 两次 `__syncthreads`，而 occupancy 只有 26%（~4 CTA/SM）。
// 本轮的思路：**让每个 warp 拥有自己的 stage 环与 scale**——W 本就按 warp_col 沿 n
// 分片（各 warp 的 W 互不相交），A 只有 16 行（80KB，常驻 L2）可每 warp 冗余读一份。
// 于是 CTA 级 `__syncthreads` 全部换成 warp 级 `__syncwarp`，`barrier` 应声而落；
// 顺带能把 STAGES 加深（无同步面），增加在飞 cp.async、抬 DRAM 利用率。
//
// 与 50 最佳（`tile_s2k8_b5` / `tile_s2k4`，K-blocked 权重 + 转置 scale）同进程对照。
// 变体开关：
//   TILED  —— 权重用 K-blocked 布局 `Wt[g][N][64B]`（一个 warp 一个 stage 的 W = 连续 2KB）
//   WMODE  —— 0=W 每 stage 载入；1=A 每 warp 一次性预载整段（消冗余 A 的 L2 流量）
//
...
//
// 运行：
//   scripts/run.sh 51-w4a8-imma-warp/w4a8_imma_warp.cu [which] [only] [chkM]
