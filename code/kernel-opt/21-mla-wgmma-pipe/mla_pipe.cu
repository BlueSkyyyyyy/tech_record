...
__global__ void __launch_bounds__(T) mla_pipe_kernel(const bf16* __restrict__ qa,
...
#pragma unroll
  ...
  for (int k0 = 0; k0 < Sk; k0 += KT) {
    ...
    for (int idx = tid; idx < KT * (DC / 8); idx += T) {
      ...
      uint4 v = *reinterpret_cast<const uint4*>(&ckv[(size_t)(k0 + kv) * DC + cu * 8]);
      sw128_store16(ks, kv, cu * 8, DK, v);
    }
    for (int idx = tid; idx < KT * (DR / 8); idx += T) {
      const int kv = idx / (DR / 8), cu = idx % (DR / 8);
      uint4 v = *reinterpret_cast<const uint4*>(&kr[(size_t)(k0 + kv) * DR + cu * 8]);
      sw128_store16(ks, kv, DC + cu * 8, DK, v);
    }
    // ---- 载入 V = c_kv^T -> vs（MN=dv, K=kv）----
    // 迭代顺序改为 dv 最快：连续线程读连续 dv -> 全局读合并（原版 kb 最快，连续线程
    // 地址差 8*DC*2 字节，完全碎片化，ncu 里表现为 47.8% 的 L1TEX stall）。
...
#pragma unroll
    ...
    // ---- 写 P（wg0 写全部 64 行，两 WG 的 S 相同）----
    if (wg == 0) {
#pragma unroll
      for (int t = 0; t < 8; ++t) {
        const int col = t * 8 + (lane & 3) * 2;
        // col 为偶数 -> sw128_off 是 4B 对齐；把 (col, col+1) 打包成一次 u32 存储
...
int main(int argc, char** argv) {
  ...
  std::printf("\nMLA wgmma pipe (coalesced V): H=%d Sq=%d Sk=%d  (BM=%d KT=%d NWG=%d)\n", H, Sq, Sk, BM,
              KT, NWG);
  ...
  std::printf("wgmma_pipe %8.4f ms  %8.2f TFLOPS (%5.1f%% peak, QK dup 2x)\n", t, tfl,
              100.0 * tfl / 989.0);
