...
__device__ __forceinline__ void decode_word(uint32_t w, uint32_t& a, uint32_t& b,
  ...
  if (DECODE == 0) {
    ...
    // 诊断：完全跳过解码（结果无意义），只为量出「解码占多少指令/时间」的天花板。
    a = w; b = w;
  } else {
...
// ===========================================================================
// 65（主题 26t）：K1 的 dp4a -> IMMA 整数张量核
//   mma.sync.aligned.m16n8k32.row.col.s32.s8.s8.s32：A = 16 个 token 的激活 int8，
//   B = 8 个输出通道的权重 int8（由 FP4 现场 PRMT 解码），C = 16x8 int32。
//   权重用 host 一次性置换：把每个 lane 需要的 8 个 fp4 nibble 装进一个 uint32
//   （nibble 顺序 = [k=kw*4..+3, k=kw*4+16..+19]），于是 decode_word_prmt 一次出 (b0,b1)。
//   E8M0 权重 scale 粒度为 32，按 chunk 折算；激活 scale 粒度为 128。
...
// 置换：Wp[N][KK/2]（每 expert 一份）-> Wperm[(N/BN)][KK/128][BN/8][4][32] uint32。
// blockIdx.y = expert。
template <int KK, int BN>
__global__ void permute_fp4_tile(const uint8_t* __restrict__ Wp, uint32_t* __restrict__ Wout,
                                 int N) {
  const int NK = KK / 128, NBLK = BN / 8, NTILE = N / BN;
  const long per_tile = (long)NK * NBLK * 128;
  const long total = (long)NTILE * per_tile;
  const size_t eb = (size_t)blockIdx.y * (size_t)N * (KK / 2);
  const size_t eo = (size_t)blockIdx.y * (size_t)total;
  for (long i = (long)blockIdx.x * blockDim.x + threadIdx.x; i < total;
       i += (long)gridDim.x * blockDim.x) {
    const int lane = i & 31;
    long r = i >> 5;
    const int chunk = r & 3; r >>= 2;
    const int nblk = (int)(r % NBLK); r /= NBLK;
    const int kblock = (int)(r % NK); r /= NK;
    const int ntile = (int)r;
    const int n = ntile * BN + nblk * 8 + (lane >> 2);
    const int g = lane & 3;
...
#pragma unroll
    for (int t = 0; t < 4; ++t) {
      const int k = base + t;
      const int nib = (Wp[eb + (size_t)n * (KK / 2) + k / 2] >> ((k & 1) * 4)) & 0xF;
      word |= (uint32_t)nib << (4 * t);
      const int k2 = base + 16 + t;
      const int nib2 = (Wp[eb + (size_t)n * (KK / 2) + k2 / 2] >> ((k2 & 1) * 4)) & 0xF;
...
template <int KK, int BN, int STAGES, int WN, int MINCTA, int DIAG = 0, int DECODE = 8>
__global__ void __launch_bounds__(WN * 32, MINCTA) k1_imma(
    const int8_t* __restrict__ Aq, const float* __restrict__ Axs,
    const uint32_t* __restrict__ Wperm, const uint8_t* __restrict__ Wsall,
    const int* __restrict__ counts, const int* __restrict__ toff,
    const int* __restrict__ toks, const int* __restrict__ wexp,
    float* __restrict__ Y, int N) {
  constexpr int MTN = (BN / WN) / 8;
  constexpr int NBLK = BN / 8;
  constexpr int NK = KK / 128;   // kblock 数 = stage 数
  constexpr int NKG = KK / 128;
  constexpr int NK32 = KK / 32;
  constexpr int ASP = 128 + 16;
  ...
  int8_t* As = reinterpret_cast<int8_t*>(sm);                                   // [STAGES][16][ASP]
  uint32_t* Ws = reinterpret_cast<uint32_t*>(As + (size_t)STAGES * 16 * ASP);   // [WN][STAGES][MTN][4][32]
  float* ssa = reinterpret_cast<float*>(Ws + (size_t)WN * STAGES * MTN * 128);  // [16][NKG]
  uint8_t* ssw = reinterpret_cast<uint8_t*>(ssa + 16 * NKG);                    // [BN][NK32]
  __shared__ uint16_t l16[L16N<DECODE>()];
  init_lut<DECODE>(l16, tid);

  for (int i = tid; i < 16 * NKG; i += WN * 32) {
    const int r = i / NKG, g = i % NKG;
    const int token = (r < m) ? toks[off + r] : -1;
    ssa[i] = (token >= 0) ? Axs[(size_t)token * NKG + g] : 0.f;
  }
  // 权重 E8M0 scale 一次进 smem（[BN][NK32]），消掉每 mma 两次 global `__ldg` 的地址算术。
  {
    const uint8_t* wbase = Wsall + (size_t)e * N * NK32;
    for (int i = tid; i < BN * NK32; i += WN * 32) {
      const int ch = i / NK32, kk = i % NK32;
      ssw[i] = wbase[(size_t)(ntile * BN + ch) * NK32 + kk];
    }
  }

  auto load_stage = [&](int st, int kb) {
    for (int i = tid; i < 16 * 8; i += WN * 32) {
      const int r = i / 8, ch = i % 8;
      const int token = (r < m) ? toks[off + r] : -1;
      int8_t* dst = As + ((size_t)st * 16 + r) * ASP + ch * 16;
      if (token >= 0)
        __pipeline_memcpy_async(dst, Aq + (size_t)token * KK + kb * 128 + ch * 16, 16);
      else
        *reinterpret_cast<uint4*>(dst) = make_uint4(0, 0, 0, 0);
    }
#pragma unroll
    for (int j = 0; j < MTN; ++j) {
      const uint32_t* src = Wperm + (size_t)e * (size_t)N * (KK / 8) +
                            (((size_t)ntile * NK + kb) * NBLK + (w * MTN + j)) * 128;
      void* dst = (char*)Ws + (((size_t)w * STAGES + st) * MTN + j) * 512 + lane * 16;
      __pipeline_memcpy_async(dst, (const char*)src + lane * 16, 16);
    }
    __pipeline_commit();
  };
...
#pragma unroll 1
  for (int t = 0; t < NK; ++t) {
    ...
    __syncthreads();
    const uint32_t* As32 = reinterpret_cast<const uint32_t*>(As + (size_t)st * 16 * ASP);
    uint32_t a[4];
    const float s0 = ssa[r0 * NKG + t];
    const float s1 = ssa[(r0 + 8) * NKG + t];
#pragma unroll
    for (int c = 0; c < 4; ++c) {
...
#pragma unroll
      for (int j = 0; j < MTN; ++j) {
        const uint32_t wd = Ws[(((size_t)w * STAGES + st) * MTN + j) * 128 + c * 32 + lane];
        uint32_t b[2];
        decode_word<DECODE>(wd, b[0], b[1], l16);
        int ac[4] = {0, 0, 0, 0};
        imma_m16n8k32(ac, a, b);
        if constexpr (DIAG & 1) {
          facc[j][0] += (float)ac[0];
          facc[j][1] += (float)ac[1];
          facc[j][2] += (float)ac[2];
          ...
          const int ch0l = (w * MTN + j) * 8 + n0;
          const int swi = t * 4 + c;
          const float sw0 = __int_as_float((int)ssw[ch0l * NK32 + swi] << 23);
          const float sw1 = __int_as_float((int)ssw[(ch0l + 1) * NK32 + swi] << 23);
          facc[j][0] += s0 * sw0 * (float)ac[0];
          facc[j][1] += s0 * sw1 * (float)ac[1];
          facc[j][2] += s1 * sw0 * (float)ac[2];
          facc[j][3] += s1 * sw1 * (float)ac[3];
        }
      }
    }
    __syncthreads();
...
#pragma unroll
  for (int j = 0; j < MTN; ++j) {
    const int ch = ntile * BN + (w * MTN + j) * 8 + n0;
    if (r0 < m) {
      Y[(size_t)(off + r0) * N + ch] = 0.5f * facc[j][0];
      Y[(size_t)(off + r0) * N + ch + 1] = 0.5f * facc[j][1];
    }
    if (r0 + 8 < m) {
      Y[(size_t)(off + r0 + 8) * N + ch] = 0.5f * facc[j][2];
      Y[(size_t)(off + r0 + 8) * N + ch + 1] = 0.5f * facc[j][3];
    }
  }
}
...
#define SMEM_IMMA(KK_, BN, STAGES, WN) \
  ((size_t)(STAGES) * 16 * 144 + (size_t)(WN) * (STAGES) * ((BN) / (WN) / 8) * 128 * 4 + \
   (size_t)16 * ((KK_) / 128) * 4 + (size_t)(BN) * ((KK_) / 32))

template <int KK, int BN, int STAGES, int WN, int MINCTA, int DIAG = 0, int DECODE = 8>
static void launch_imma(dim3 grid, int smem, const int8_t* Aq, const float* Axs,
                        const uint32_t* Wperm, const uint8_t* Ws, const int* counts,
                        const int* toff, const int* toks, const int* wexp, float* Y, int N) {
  auto k = k1_imma<KK, BN, STAGES, WN, MINCTA, DIAG, DECODE>;
  cudaError_t e = cudaFuncSetAttribute((const void*)k, cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
  if (e != cudaSuccess) { std::fprintf(stderr, "IMMA setattr smem=%d failed: %s\n", smem, cudaGetErrorString(e)); std::exit(1); }
  k<<<grid, WN * 32, smem>>>(Aq, Axs, Wperm, Ws, counts, toff, toks, wexp, Y, N);
}
...
#define K2_TMA(D_, M_) do { constexpr int RW = (M_) == 1 ? 8 : 4; constexpr int NW = (M_) == 4 ? 4 : 8; launch_tma<I, RW, NW, M_, D_, 0>(dim3(N2 / (NW * RW), R.n_active), SMEM_TMA(I, RW, NW, M_), dHq, dHs, dB2, dBs2, dCounts, dToff, dToks, dWexp, dO, N2, HINT, ((EK >> 1) & 1)); } while (0)
...
#undef SW2
  ...
  // ---- bg 模式：大 batch (m>1) 的 TMA (RWW,NWARP) 几何 + 解码方式联合扫描 ----
  if (!std::strcmp(mode, "bg")) {
    if (B <= 64) { std::printf("bg mode only for B>64\n"); return 0; }
    auto run1 = [&](auto mt, auto dec, auto rw, auto nw) {
      constexpr int M_ = decltype(mt)::value, D_ = decltype(dec)::value,
                    RW = decltype(rw)::value, NW = decltype(nw)::value;
      const int sm = SMEM_TMA(H, RW, NW, M_);
      dim3 g(N1 / (NW * RW), R.n_active);
      double t = bench_ms([&] { launch_tma<H, RW, NW, M_, D_, 1>(g, sm, dXq, dXs, dB13, dBs13,
                          dCounts, dToff, dToks, dWexp, dGU, N1, HINT, (EK & 1)); }, 3, 30);
      std::printf("  K1 MT%d DEC%d RWW%d NW%d  %.4f ms  %.1f GB/s (%.1f%%)  smem=%dKB\n",
                  M_, D_, RW, NW, t, to_gbps(b1, t), 100 * to_gbps(b1, t) / di.mem_bw_gbps, sm / 1024);
    };
    auto run2 = [&](auto mt, auto dec, auto rw, auto nw) {
      constexpr int M_ = decltype(mt)::value, D_ = decltype(dec)::value,
                    RW = decltype(rw)::value, NW = decltype(nw)::value;
      const int sm = SMEM_TMA(I, RW, NW, M_);
      dim3 g(N2 / (NW * RW), R.n_active);
      double t = bench_ms([&] { launch_tma<I, RW, NW, M_, D_, 0>(g, sm, dHq, dHs, dB2, dBs2,
                          dCounts, dToff, dToks, dWexp, dO, N2, HINT, ((EK >> 1) & 1)); }, 3, 30);
      std::printf("  K2 MT%d DEC%d RWW%d NW%d  %.4f ms  %.1f GB/s (%.1f%%)  smem=%dKB\n",
                  M_, D_, RW, NW, t, to_gbps(b2, t), 100 * to_gbps(b2, t) / di.mem_bw_gbps, sm / 1024);
    };
    const bool m4 = (MT >= 4);
#define BG1(D_, RW_, NW_) do { if (m4) run1(std::integral_constant<int,4>{}, std::integral_constant<int,D_>{}, std::integral_constant<int,RW_>{}, std::integral_constant<int,NW_>{}); \
                               else    run1(std::integral_constant<int,2>{}, std::integral_constant<int,D_>{}, std::integral_constant<int,RW_>{}, std::integral_constant<int,NW_>{}); } while (0)
#define BG2(D_, RW_, NW_) do { if (m4) run2(std::integral_constant<int,4>{}, std::integral_constant<int,D_>{}, std::integral_constant<int,RW_>{}, std::integral_constant<int,NW_>{}); \
                               else    run2(std::integral_constant<int,2>{}, std::integral_constant<int,D_>{}, std::integral_constant<int,RW_>{}, std::integral_constant<int,NW_>{}); } while (0)
    std::printf("== bg K1 (H=7168) ==\n");
    BG1(4, 2, 8); BG1(8, 2, 8); BG1(4, 1, 8); BG1(8, 1, 8); BG1(4, 2, 4); BG1(8, 2, 4);
    std::printf("== bg K2 (I=3072) ==\n");
    BG2(4, 4, 4); BG2(8, 4, 4); BG2(4, 8, 8); BG2(8, 8, 8); BG2(4, 4, 8); BG2(8, 4, 8); BG2(4, 2, 8); BG2(8, 2, 8);
#undef BG1
#undef BG2
    CUDA_CHECK_LAST();
    CUDA_CHECK(cudaDeviceSynchronize());
...
#undef T2
  ...
  // ---- imma 模式：K1 的 dp4a -> IMMA 张量核（同进程与 baseline K1 对拍 + 计时） ----
  if (!std::strcmp(mode, "imma")) {
    constexpr int BN = 128;
    uint32_t* dW13;
    CUDA_CHECK(cudaMalloc(&dW13, (size_t)E_EXP * b13p.size()));
    {
      dim3 pg(1024, E_EXP);
      permute_fp4_tile<H, BN><<<pg, 256>>>(dB13, dW13, N1);
    }
    CUDA_CHECK_LAST();
    CUDA_CHECK(cudaDeviceSynchronize());
    double tp = bench_ms([&] { dim3 pg(1024, E_EXP); permute_fp4_tile<H, BN><<<pg, 256>>>(dB13, dW13, N1); }, 1, 3);
    std::printf("permute w1/w3 fp4 -> IMMA layout: %.3f ms (%.1f GB/s)\n", tp,
                to_gbps((double)E_EXP * b13p.size() * 2, tp));

    // baseline K1（cp.async gemv，DEC 由 argv 控制，建议 DEC=8）
    auto run_base = [&] {
      if (KTMA == 1) { if (DEC == 0) DISPATCH_K1T(0); else if (DEC == 4) DISPATCH_K1T(4); else DISPATCH_K1T(8); }
      else { if (DEC == 0) DISPATCH_K1(0); else if (DEC == 4) DISPATCH_K1(4); else DISPATCH_K1(8); }
    };
    run_quant();
    run_base();
    CUDA_CHECK_LAST();
    CUDA_CHECK(cudaDeviceSynchronize());
    float* dGUb;
    CUDA_CHECK(cudaMalloc(&dGUb, (size_t)pairs * N1 * 4));
    CUDA_CHECK(cudaMemcpy(dGUb, dGU, (size_t)pairs * N1 * 4, cudaMemcpyDeviceToDevice));

#define RUN_IMMA(ST_, WN_) do { \
    dim3 g(N1 / BN, R.n_active); \
    int sm = (int)SMEM_IMMA(H, BN, ST_, WN_); \
    launch_imma<H, BN, ST_, WN_, 1>(g, sm, dXq, dXs, dW13, dBs13, dCounts, dToff, dToks, dWexp, dGU, N1); \
  } while (0)

    // 正确性：一个配置 vs baseline
    CUDA_CHECK(cudaMemset(dGU, 0, (size_t)pairs * N1 * 4));
    RUN_IMMA(3, 8);
    CUDA_CHECK_LAST();
    CUDA_CHECK(cudaDeviceSynchronize());
    {
      std::vector<float> hA_((size_t)pairs * N1), hB_((size_t)pairs * N1);
      CUDA_CHECK(cudaMemcpy(hA_.data(), dGU, hA_.size() * 4, cudaMemcpyDeviceToHost));
      CUDA_CHECK(cudaMemcpy(hB_.data(), dGUb, hB_.size() * 4, cudaMemcpyDeviceToHost));
      double mr = 0, mref = 0;
      for (size_t i = 0; i < hA_.size(); ++i) {
        mr = std::max(mr, std::fabs((double)hA_[i] - hB_[i]) / std::max(std::fabs((double)hB_[i]), 1.0));
        mref = std::max(mref, std::fabs((double)hB_[i]));
      }
      std::printf("[check K1 imma vs baseline] max_rel=%.3e (ref~%.2f) %s\n", mr, mref,
                  mr < 3e-2 ? "OK" : "FAIL");
    }

    auto bench_imma = [&](auto dec, auto st, auto wn) {
      constexpr int D_ = decltype(dec)::value, ST = decltype(st)::value, WN = decltype(wn)::value;
      dim3 g(N1 / BN, R.n_active);
      int sm = (int)SMEM_IMMA(H, BN, ST, WN);
      double t = bench_ms([&] { launch_imma<H, BN, ST, WN, 1, 0, D_>(g, sm, dXq, dXs, dW13, dBs13,
                           dCounts, dToff, dToks, dWexp, dGU, N1); }, 3, 30);
      std::printf("  K1 IMMA DEC%d BN%d ST%d WN%d (MTN%d)  %.4f ms  %.1f GB/s (%.1f%% HBM)  smem=%dKB\n",
                  D_, BN, ST, WN, (BN / WN) / 8, t, to_gbps(b1, t),
                  100 * to_gbps(b1, t) / di.mem_bw_gbps, sm / 1024);
    };
    std::printf("== K1 IMMA sweep (BN=%d) ==\n", BN);
    bench_imma(std::integral_constant<int, 8>{}, std::integral_constant<int, 2>{}, std::integral_constant<int, 8>{});
    bench_imma(std::integral_constant<int, 8>{}, std::integral_constant<int, 3>{}, std::integral_constant<int, 8>{});
    bench_imma(std::integral_constant<int, 8>{}, std::integral_constant<int, 4>{}, std::integral_constant<int, 8>{});
    bench_imma(std::integral_constant<int, 4>{}, std::integral_constant<int, 4>{}, std::integral_constant<int, 8>{});
    bench_imma(std::integral_constant<int, 0>{}, std::integral_constant<int, 4>{}, std::integral_constant<int, 8>{});
    bench_imma(std::integral_constant<int, 8>{}, std::integral_constant<int, 4>{}, std::integral_constant<int, 4>{});
    bench_imma(std::integral_constant<int, 4>{}, std::integral_constant<int, 3>{}, std::integral_constant<int, 8>{});

    // 诊断：短路解码（DECODE=99）/ 短路折算（DIAG=1），看各自值多少（结果无意义）
    auto diag_imma = [&](int tag, auto st, auto wn) {
      constexpr int ST = decltype(st)::value, WN = decltype(wn)::value;
      dim3 g(N1 / BN, R.n_active);
      int sm = (int)SMEM_IMMA(H, BN, ST, WN);
      double t = bench_ms([&] {
        if (tag == 1) launch_imma<H, BN, ST, WN, 1, 1, 8>(g, sm, dXq, dXs, dW13, dBs13, dCounts, dToff, dToks, dWexp, dGU, N1);
        else if (tag == 2) launch_imma<H, BN, ST, WN, 1, 0, 99>(g, sm, dXq, dXs, dW13, dBs13, dCounts, dToff, dToks, dWexp, dGU, N1);
        else launch_imma<H, BN, ST, WN, 1, 1, 99>(g, sm, dXq, dXs, dW13, dBs13, dCounts, dToff, dToks, dWexp, dGU, N1);
      }, 3, 30);
      std::printf("  DIAG%d (fold%s decode%s)  %.4f ms  %.1f GB/s (%.1f%% HBM)\n",
                  tag, (tag & 1) ? "-off" : "", (tag & 2) ? "-off" : "", t, to_gbps(b1, t),
                  100 * to_gbps(b1, t) / di.mem_bw_gbps);
    };
    diag_imma(1, std::integral_constant<int, 4>{}, std::integral_constant<int, 8>{});
    diag_imma(2, std::integral_constant<int, 4>{}, std::integral_constant<int, 8>{});
    diag_imma(3, std::integral_constant<int, 4>{}, std::integral_constant<int, 8>{});
#undef RUN_IMMA

    const double tb = bench_ms(run_base, 3, 30);
    std::printf("  K1 baseline (DEC=%d)  %.4f ms  %.1f GB/s (%.1f%% HBM)\n", DEC, tb,
                to_gbps(b1, tb), 100 * to_gbps(b1, tb) / di.mem_bw_gbps);
    CUDA_CHECK_LAST();
    CUDA_CHECK(cudaDeviceSynchronize());
    return 0;
  }
  ...
  auto run_k1 = [&] {
    if (KTMA == 1) {
      if (DEC == 0) { DISPATCH_K1T(0); } else if (DEC == 4) { DISPATCH_K1T(4); } else if (DEC == 99) { DISPATCH_K1T(99); } else { DISPATCH_K1T(8); }
      return;
    }
  ...
  auto run_k2 = [&] {
    if (KTMA >= 1) {
      if (DEC == 0) { DISPATCH_K2T(0); } else if (DEC == 4) { DISPATCH_K2T(4); } else if (DEC == 99) { DISPATCH_K2T(99); } else { DISPATCH_K2T(8); }
      return;
    }
