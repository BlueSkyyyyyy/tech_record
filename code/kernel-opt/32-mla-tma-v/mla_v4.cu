...
#pragma unroll
  ...
  // ---- prologue V: 向量化 global 读（8 连续 dv）× 8 key，8 次标量 smem 写 ----
  for (int i = tid; i < KT * (DV / 8); i += T) {
    const int k = i / (DV / 8), d8 = (i % (DV / 8)) * 8;
    const uint4 raw = *reinterpret_cast<const uint4*>(&ckv[(size_t)k * DC + d8]);
    const unsigned short* sp = reinterpret_cast<const unsigned short*>(&raw);
#pragma unroll
    for (int r = 0; r < 8; ++r)
      *reinterpret_cast<unsigned short*>(vs + sw128_off(d8 + r, k, KT)) = sp[r];
  }
  __syncthreads();
...
#pragma unroll
    ...
    if (has_next) {
      for (int i = tid; i < KT * (DV / 8); i += T) {
        const int k = i / (DV / 8), d8 = (i % (DV / 8)) * 8;
        const uint4 raw = *reinterpret_cast<const uint4*>(&ckv[(size_t)(k0 + KT + k) * DC + d8]);
        const unsigned short* sp = reinterpret_cast<const unsigned short*>(&raw);
#pragma unroll
        for (int r = 0; r < 8; ++r)
          *reinterpret_cast<unsigned short*>(vs + sw128_off(d8 + r, k, KT)) = sp[r];
