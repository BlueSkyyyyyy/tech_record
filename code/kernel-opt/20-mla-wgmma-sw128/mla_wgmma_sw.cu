...
__global__ void __launch_bounds__(T) mla_wgmma_sw_kernel(const bf16* __restrict__ qa,
...
int main(int argc, char** argv) {
  ...
  std::printf("\nMLA wgmma SW128 fused: H=%d Sq=%d Sk=%d  (BM=%d KT=%d NWG=%d)\n", H, Sq, Sk, BM,
              KT, NWG);
  ...
  std::printf("wgmma_sw128 %8.4f ms  %8.2f TFLOPS (%5.1f%% peak, QK dup 2x)\n", t, tfl,
              100.0 * tfl / 989.0);
