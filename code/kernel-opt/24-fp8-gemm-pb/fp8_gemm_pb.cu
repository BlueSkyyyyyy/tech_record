...
#pragma unroll
      for (int i = 0; i < 64; ++i) acc[c][j][i] = 0.f;
#pragma unroll
  for (int j = 0; j < NSPLIT; ++j)
#pragma unroll
...
#pragma unroll
      for (int j = 0; j < NSPLIT; ++j)
#pragma unroll
        for (int i = 0; i < 64; ++i) { acc[SET][j][i] = 0.f; }
    }
    wgmma_fence();
#pragma unroll
    for (int jn = 0; jn < NSPLIT; ++jn) {
...
#pragma unroll
      for (int s = 0; s < BK / 32; ++s) {
        ...
        wgmma_m64n128k32(acc[SET][jn], da, db);
      }
    }
    wgmma_commit();
  };
  ...
  if constexpr (MODE == 1) {
    // 每轮处理 2 个 k 块（set0 / set1），set 是编译期常量；折算上一块与当前块 wgmma 重叠。
    int kb = 0;
    for (; kb + 1 < nblk; kb += 2) {
      issue(S0, kb);
...
#pragma unroll
    for (int j = 0; j < NSPLIT; ++j)
#pragma unroll
      for (int i = 0; i < 64; ++i) acc[0][j][i] = 0.f;
    for (int kb = 0; kb < nblk; ++kb) {
      issue(S0, kb);
      if (kb >= STAGES - 2) {
