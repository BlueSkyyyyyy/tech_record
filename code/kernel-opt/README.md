...
| `34-fused-moe-fp8/` | 34 fused MoE + per-block FP8 | DeepSeek-V4-Pro 真实 shape（hidden=7168, inter=3072, 384 experts top-6, e4m3 + ue8m0 + `weight_block=128×128`）。`moe_fp8.cu`：把 31 的 grouped per-block GEMM（TMA+mbarrier+WS+`wgmma` k32+SW128，每 128-k 块 `fin+=sa*sb*acc`）接进 30 的 MoE FFN 端到端，`UNPERM` 模板切「down 是否融 unpermute」。端到端 **11.78ms @M8k bal / 20.58 @M16k / 15.93 @M8k rand**，相对 30 篇 bf16 **1.92×/2.21×/2.06×**；专家权重 50.7→**25.4GB**；K2 down **88.9% HBM**（ncu DRAM 89.3%）、K1 78%（DRAM 80.1%、`long_scoreboard` 占 stall 51.3%、occ 13.9%）。两个负结果：**unpermute 融合在 FP8 下反向**（K2f 3.83 vs 3.35+0.54，ncu L2 **94.4%**/DRAM 73.2%，散写不再被藏住）、**per-block + 双累加器 SwiGLU = 256>255 寄存器**（`BN=256` 崩到 21.7ms）。附带 `swiglu_quant_v2`（一行一 block + `float4` + warp 内 `__shfl_xor` amax）**0.44ms / 3090 GB/s / 92% HBM**（v1 39%）。含 `moe_fp8_M{8192,16384}_{bal,rand}.out.txt`、`ncu_k1/k2/k2f.out.txt` |
