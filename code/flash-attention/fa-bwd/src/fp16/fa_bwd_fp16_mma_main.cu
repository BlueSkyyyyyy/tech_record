...
static int run_varlen(const std::string& dir, bool causal, int iters, int varlen_tma = 0,
  ...
  //   每 scheduler warp 数 1→2（对齐 O46/O47 的通用杠杆），且 grid 与「每-C 读 K 次数」减半。
  //   仅服务 D=512 && 非 causal（O54 FULL 路径）；`--lse8w=0/1` 同 binary A/B。
  const bool lse8w_use = (lse8w != 0) && (D == 512) && !causal;
  const int lse_nblk8 = (maxlen + 127) / 128;   // 8-warp 版 LBM=128 的 m 块数
  ...
  const int kLseSmemBal1_8 = (128 + 2 * 32) * (D + 8) * (int)sizeof(__half);
  const int kLseSmemBal1_8c = (128 + 64) * (D + 8) * (int)sizeof(__half);   // 8-warp, LBN=64, PIPE=0
  ...
  if (D == 128) {
    ...
    CUDA_CHECK(cudaFuncSetAttribute(lse_mma_kernel_bal<512, 0, true, 256, 64>,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize, kLseSmemBal1_8c));
  }
  ...
  auto run_all = [&]() {
    ...
    if (D == 128) {
      ...
      if (causal) {
        ...
        // O56：full MLA varlen 的 LSE 走 8-warp（256 线程 / LBM=128 / LBN=32）几何——每
        //   scheduler warp 数 1→2、grid/每-C 读 K 减半；仍带 O54 的 K 维 split。
        if (lse_split_eff > 1) {
  ...
  if (D == 512 && !causal) {
    auto go_lse = [&](int which, int sp) {
      if (which == 0) {
        ...
        // O56 备选：8-warp / LBN=64 / PIPE=0（单缓冲 K，tile 数与 4-warp 相同）。
        if (sp <= 1) {
          lse_mma_kernel_bal<512, 0, true, 256, 64>
              <<<dim3(lse_nblk8, H, B), 256, kLseSmemBal1_8c>>>(d_q, d_k, d_lse, maxlen, H, Hkv,
                                                                scale, d_cu);
        } else {
          dim3 gsp((unsigned)lse_nblk8, H, (unsigned)(B * sp));
          lse_mma_kernel_bal<512, 0, true, 256, 64><<<gsp, 256, kLseSmemBal1_8c>>>(
              d_q, d_k, d_lse, maxlen, H, Hkv, scale, d_cu, d_lse_part, sp);
          const long long nrows = (long long)rows_q;
          const int th = 256;
          lse_split_merge_kernel<<<(unsigned)((nrows + th - 1) / th), th>>>(d_lse_part, d_lse,
    ...
    printf("[O56 A/B] ... 8-warp LBN64/PIPE0:");
    for (int sp : {1, 2, 4, 8, 16}) {
      float t = 0.f;
      bench_lse(3, sp, &t);
      printf(" split%d %.4f", sp, t);
...
int main(int argc, char** argv) {
  ...
  int mla8w = 1;
  // O56：full MLA（D=512）varlen 的 LSE 是否用 8-warp（256 线程/LBM=128/LBN=32）几何——同
  //   1 CTA/SM 下每 scheduler warp 数 1→2（对齐 O46/O47），grid/每-C 读 K 减半。
  //   **实测为混合/负结果**（LSE-only：长 K 的 b3 1.09×、短 K 的 b1 0.80×；端到端 +0.8%/−9.7%）
  //   ⇒ **默认 0（opt-in）**，`--lse8w=1` 开启、`--lse8w=0` 关（同 binary A/B）。
  int lse8w = 0;
