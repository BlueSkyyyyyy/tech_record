// =============================================================================
// fa_bwd_bf16_mma_onefile.cu —— FlashAttention 反向（bf16）**张量核单文件版**（O5b/O6c）
// =============================================================================
...
//
// O6c：主 kernel 的 2×2 warp 几何由 (BM,BN) 参数化，host 按网格自动选 tile；与 fp16 版
// 逐字同构，详见 `docs/01` §13b 与 ROADMAP O6c。
//
...
// O7c：R4（dK/dV float4 归约，默认关）与 PREL（LSE/D 预装寄存器，默认开）。
template <int HD, int BM, int BN, int PIPE, bool R4 = false, bool PREL = true>
...
int main(int argc, char** argv) {
  ...
  constexpr int kLseSmemBal0 = (LBM + LBN) * (128 + 8) * (int)sizeof(bf16);
  constexpr int kLseSmemBal1 = (LBM + 2 * LBN) * (128 + 8) * (int)sizeof(bf16);
