...
## 2. 纯反向性能：FA2 vs FA3 vs TE（CUPTI 纯 device 时间，bf16 与 fp16 一致）

> **单位说明**：下表主指标是**时间**（µs，越小越好）。`TF`/`TFLOPS` 是「每秒 Tera（10¹²）次浮点运算」，
> 是**吞吐/算力**单位、**不是时间**；它由 `FLOPs ÷ 时间` 换算而来，仅作参考。本文尽量以时间为准。

| 形状 (B,S,H,D) | FA2.7.4 (SM80) | **FA3 (SM90)** | TE2.14 (cuDNN SM90) | FA3/FA2（时间比） |
|---|---|---|---|---|
...
（含 TFLOPS 的原始表见 `src/fa2_fa3_te_bwd_only.out.txt`；同 shape 的 `TFLOPS = 4·B·S²·H·D ÷ 时间`。）
